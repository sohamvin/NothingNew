...
class Order:
    def __init__(self, order_id: str, time: datetime, order_type: str, quantity: int, price: float, company_id: str, user_id: str = None):
        ...
        if time.tzinfo is None:
            self.time = time.replace(tzinfo=ist)
        else:
            self.time = time.astimezone(ist)
...
for i in range(UPPER_LIMIT):  # Starting from 4 to 23 for unique order IDs
    order_type = orders_arrival[i][0]
    quantity = orders_arrival[i][2]  # Random quantity between 1 and 20
    price = orders_arrival[i][1]  # Random price between 250.0 and 400.0
    company_id = "AAPL"  # Assuming all orders are for the same stock for simplicity

    additional_orders.append(Order(str(i), datetime.now(), order_type, quantity, price, company_id))


for o in additional_orders:
